import Link from "next/link";
import type { Property } from "@/lib/types";

interface FooterProps {
  property: Property;
}

export default function Footer({ property }: FooterProps) {
  const { social_links } = property;
  const hasSocials =
    social_links?.instagram ||
    social_links?.facebook ||
    social_links?.tripadvisor ||
    social_links?.google_maps;

  return (
    <footer className="bg-primary-900 text-white/70">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Main grid */}
        <div className="py-16 lg:py-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-6">
              {property.logo ? (
                <img src={property.logo} alt={property.name} className="h-10 w-auto object-contain" />
              ) : (
                <div className="w-10 h-10 rounded bg-white/10 flex items-center justify-center">
                  <span className="text-white font-serif text-lg font-bold">
                    {property.name.charAt(0)}
                  </span>
                </div>
              )}
              <span className="text-white font-serif text-xl tracking-wide">
                {property.name}
              </span>
            </div>
            <p className="text-sm leading-relaxed text-white/50 mb-6">
              {property.address}
              <br />
              {property.city}, {property.country}
            </p>

            {hasSocials && (
              <div className="flex items-center gap-2.5">
                {social_links.instagram && (
                  <a
                    href={social_links.instagram}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-accent-600 hover:text-white transition-all duration-300"
                    aria-label="Instagram"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                  </a>
                )}
                {social_links.facebook && (
                  <a
                    href={social_links.facebook}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-accent-600 hover:text-white transition-all duration-300"
                    aria-label="Facebook"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  </a>
                )}
                {social_links.tripadvisor && (
                  <a
                    href={social_links.tripadvisor}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-accent-600 hover:text-white transition-all duration-300"
                    aria-label="TripAdvisor"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12.006 4.295c-2.67 0-5.338.784-7.645 2.353H0l1.963 2.135a5.997 5.997 0 004.04 10.43 5.976 5.976 0 004.075-1.6L12 19.705l1.922-2.09a5.976 5.976 0 004.075 1.6 5.997 5.997 0 004.04-10.43L24 6.648h-4.35a13.573 13.573 0 00-7.644-2.353zM6.003 17.213a3.997 3.997 0 110-7.994 3.997 3.997 0 010 7.994zm11.994 0a3.997 3.997 0 110-7.994 3.997 3.997 0 010 7.994zM6.003 11.219a2 2 0 100 4 2 2 0 000-4zm11.994 0a2 2 0 100 4 2 2 0 000-4z" />
                    </svg>
                  </a>
                )}
                {social_links.google_maps && (
                  <a
                    href={social_links.google_maps}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-accent-600 hover:text-white transition-all duration-300"
                    aria-label="Google Maps"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                    </svg>
                  </a>
                )}
              </div>
            )}
          </div>

          {/* Navigation */}
          <div>
            <h4 className="text-white font-sans text-xs font-semibold uppercase tracking-[0.2em] mb-6">
              Navegación
            </h4>
            <ul className="space-y-3">
              <li>
                <Link href="/" className="text-sm hover:text-accent-400 transition-colors duration-200">
                  Inicio
                </Link>
              </li>
              <li>
                <Link href="/habitaciones" className="text-sm hover:text-accent-400 transition-colors duration-200">
                  Habitaciones
                </Link>
              </li>
              <li>
                <Link href="/disponibilidad" className="text-sm hover:text-accent-400 transition-colors duration-200">
                  Disponibilidad
                </Link>
              </li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-white font-sans text-xs font-semibold uppercase tracking-[0.2em] mb-6">
              Contacto
            </h4>
            <ul className="space-y-3.5 text-sm">
              {property.contact_phone && (
                <li className="flex items-center gap-2.5">
                  <svg className="w-4 h-4 text-accent-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                  </svg>
                  {property.contact_phone}
                </li>
              )}
              {property.whatsapp && (
                <li className="flex items-center gap-2.5">
                  <svg className="w-4 h-4 text-accent-500 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  <a
                    href={`https://wa.me/${property.whatsapp.replace(/[^0-9]/g, "")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-accent-400 transition-colors duration-200"
                  >
                    {property.whatsapp}
                  </a>
                </li>
              )}
              {property.contact_email && (
                <li className="flex items-center gap-2.5">
                  <svg className="w-4 h-4 text-accent-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
                  {property.contact_email}
                </li>
              )}
            </ul>
          </div>

          {/* Hours & Info */}
          <div>
            <h4 className="text-white font-sans text-xs font-semibold uppercase tracking-[0.2em] mb-6">
              Horarios
            </h4>
            <ul className="space-y-3 text-sm">
              <li>
                <span className="text-white/40">Check-in:</span>{" "}
                {property.check_in_time}
              </li>
              <li>
                <span className="text-white/40">Check-out:</span>{" "}
                {property.check_out_time}
              </li>
            </ul>

            {property.languages.length > 0 && (
              <div className="mt-8">
                <h4 className="text-white font-sans text-xs font-semibold uppercase tracking-[0.2em] mb-3">
                  Idiomas
                </h4>
                <p className="text-sm">{property.languages.join(", ")}</p>
              </div>
            )}

            {property.payment_methods.length > 0 && (
              <div className="mt-8">
                <h4 className="text-white font-sans text-xs font-semibold uppercase tracking-[0.2em] mb-3">
                  Métodos de pago
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {property.payment_methods.map((method) => (
                    <span
                      key={method}
                      className="text-xs bg-white/10 rounded px-2.5 py-1"
                    >
                      {method}
                    </span>
                  ))}
                </div>
              </div>
            )}
          </div>
        </div>

        {/* Bottom bar */}
        <div className="border-t border-white/10 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/30">
            &copy; {new Date().getFullYear()} {property.name}. Todos los derechos reservados.
          </p>
          <p className="text-xs text-white/25">
            Powered by{" "}
            <span className="text-accent-500/60">Austin OS</span>
          </p>
        </div>
      </div>
    </footer>
  );
}
